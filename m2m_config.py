...
class CAPGConfig(object):
    """capg config"""
    def __init__(self, m_type, fold_num, seg_len):
        ...
        self.all_json_path = os.path.join(self.json_dir, 'all-f4', fold_num, 'train_list.json')
        self.train_json_path = os.path.join(self.json_dir, m_type, fold_num, 'train_list.json')
        self.test_json_path = os.path.join(self.json_dir, m_type, fold_num, 'test_list.json')
        ...
        self.batch_size = 32
        self.num_steps = int(seg_len)
        self.test_num_steps = int(seg_len)
        self.max_epoch = 20
        self.lr_decay = 1
...
def get_config(m_type, fold_num, seg_len):
    config = CAPGConfig(m_type, fold_num, seg_len)
    return config
