...
def run_all(config):
    ...
    [mus_seq, mus_all] = load_data(config.mus_data_dir, config.train_json_path, mot_scale)

    [test_mus_seq, test_mus_all] = load_data(config.mus_data_dir, config.test_json_path, mot_scale)

    print('--Loading motion data..')

    [mot_seq, mot_all] = load_data(config.mot_data_dir, config.train_json_path, mot_scale)
    [test_mot_seq, test_mot_all] = load_data(config.mot_data_dir, config.test_json_path, mot_scale)

    if config.is_all_norm:
        ...
        mot_all_data = mot_all
        mus_all_data = mus_all
    ...
    mus_seq_tensor, mot_seq_tensor = \
        get_seq_tensor(mus_seq, mus_data_max, mus_data_min,
                       mot_seq, mot_data_max, mot_data_min,
                       mot_data_mean, mot_data_std, config)

    [mus_x_tensor, mot_x_tensor, mot_y_tensor] = \
        sample_seq_idx(mus_seq, mot_seq, config)

    test_mus_seq_tensor, test_mot_seq_tensor = \
        get_seq_tensor(test_mus_seq, mus_data_max, mus_data_min,
                       test_mot_seq, mot_data_max, mot_data_min,
                       mot_data_mean, mot_data_std, config)
    ...
    data_info['train'] = [mus_seq_tensor, mot_seq_tensor, mus_x_tensor, mot_x_tensor, mot_y_tensor]
